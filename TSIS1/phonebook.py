...
def create_table():
    ...
    try:
        ...
        print("Trying to load:", schema_path)

        with open(schema_path, 'r', encoding='utf-8') as f:
            sql = f.read()
            print("SQL LOADED (preview):\n", sql[:100])
        ...
        cur.execute("""
            ...
            BEGIN
                RETURN QUERY
                SELECT phonebook2.id, phonebook2.first_name, phonebook2.second_name, phonebook2.email 
                FROM phonebook2
                ORDER BY phonebook2.id
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                SELECT phonebook2.id, phonebook2.first_name, phonebook2.second_name, phonebook2.email 
                FROM phonebook2 
                WHERE phonebook2.first_name ILIKE '%' || p || '%'
                   OR phonebook2.second_name ILIKE '%' || p || '%'
                   OR phonebook2.email ILIKE '%' || p || '%';
            END;
            $$ LANGUAGE plpgsql;
        """)
        ...
        cur.execute("""
            ...
            BEGIN
                ...
                ELSE
                    ...
                    INSERT INTO phonebook2 (first_name, second_name) 
    ...
    try:
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                cur.execute("""
                    INSERT INTO phonebook2 (first_name, second_name, email, birthday, group_id) 
...
def change_name_or_phone():
    ...
    if a == "1":
        ...
        cur.execute("""UPDATE phonebook2 SET first_name = %s, second_name = %s 
        ...
        cur.execute("""UPDATE phones SET phone = %s 
                       WHERE contact_id IN (SELECT id FROM phonebook2 WHERE first_name = %s AND second_name = %s)""", 
...
def querying_data_from_the_table():
    ...
    if a == 1:
        ...
        phonenumber = input()
        cur.execute("SELECT c.first_name, c.second_name FROM phonebook2 c JOIN phones p ON c.id = p.contact_id WHERE p.phone = %s", (phonenumber,))
        row = cur.fetchone()
        ...
        val = input()
        cur.execute(f"SELECT p.phone FROM phones p JOIN phonebook2 c ON c.id = p.contact_id WHERE c.{col} = %s", (val,))
        rows = cur.fetchall()
...
def deleting_data_from_table():
    ...
    if a == 1:
        ...
        first = input()
        cur.execute("DELETE FROM phonebook2 WHERE first_name = %s", (first,))
        conn.commit()
    elif a == 2:
        print("Enter second name:")
        second = input()
        cur.execute("DELETE FROM phonebook2 WHERE second_name = %s", (second,))
        conn.commit()
    else:
        print("Enter phone number:")
        phonenumber = input()
        cur.execute("DELETE FROM phonebook2 WHERE id IN (SELECT contact_id FROM phones WHERE phone = %s)", (phonenumber,))
        conn.commit()
...
def filter_by_group():
    ...
    g_name = input()
    cur.execute("SELECT c.first_name, c.second_name, c.email FROM phonebook2 c JOIN groups g ON c.group_id = g.id WHERE g.name ILIKE %s", (g_name,))
    for row in cur.fetchall(): print(row)

def search_by_email():
    print("Enter part of email:")
    email_q = input()
    cur.execute("SELECT first_name, second_name, email FROM phonebook2 WHERE email ILIKE %s", ('%' + email_q + '%',))
    for row in cur.fetchall(): print(row)
...
def sort_results():
    ...
    cur.execute(f"SELECT first_name, second_name, email FROM phonebook2 ORDER BY {col}")
    for row in cur.fetchall(): print(row)
...
def import_from_json():
    ...
    for c in contacts:
        cur.execute("SELECT id FROM phonebook2 WHERE first_name = %s AND second_name = %s", (c['first_name'], c['second_name']))
        exists = cur.fetchone()
        ...
        if action == 'o' and exists:
            cur.execute("UPDATE phonebook2 SET email = %s, birthday = %s, group_id = %s WHERE id = %s", 
                        (c.get('email'), c.get('birthday'), group_id, exists[0]))
            contact_id = exists[0]
        else:
            cur.execute("INSERT INTO phonebook2 (first_name, second_name, email, birthday, group_id) VALUES (%s, %s, %s, %s, %s) RETURNING id", 
